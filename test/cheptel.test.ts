...
describe("cheptelAccounting", () => {
  it("splits usable supply from owned/sink, and folds succès into obtained + the sink only", () => {
    const acc = cheptelAccounting({
      ...
      targetGen: 3, level: 100, optima: true, clonage: true,
    });
  ...
  it("an unlocked succès satisfies a colour's sink, dropping it from plan demand", () => {
    ...
    const withSucces = cheptelAccounting({ mounts: [], achievements: allUpToGen2, targetGen: 2, level: 100, optima: true, clonage: true });
    const without = cheptelAccounting({ mounts: [], achievements: [], targetGen: 2, level: 100, optima: true, clonage: true });
