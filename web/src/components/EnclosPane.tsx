...
function NumInput({ value, max, onCommit }: { value: number; max: number; onCommit: (n: number) => void }) {
  ...
  return (
    <div className="fuel-cell">
      <div className="fuel-row">
        <label className="chk" title={frozen ? "Frozen — all dragodindes maxed" : "Check to activate"}>
          <input type="checkbox" checked={checked} onChange={(e) => onToggleFocus(bar.key, e.target.checked)} />
        </label>
        <span className="fuel-label" style={{ color: bar.color }}>
          {bar.label}
        </span>
        <FuelMeter value={value} color={bar.color} active={ticking} onSet={(n) => onFuel(bar.key, n)} />
        ...
        <div className="carburant">
          <div className="carb-head">
            <input
              ...
              key={active.name}
              onBlur={(ev) => {
                if (ev.target.value !== active.name) onEnclosPatch(active.id, { name: ev.target.value });
              }}
            />
            {enclos.length > 1 && (
              <button
                ...
                onClick={() => {
                  if (window.confirm(`Delete ${active.name} and its dragodindes?`)) onEnclosDelete(active.id);
                }}
              >
                ✕
              </button>
