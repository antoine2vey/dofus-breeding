    ...
    <div className="mount-editor">
      ...
      <label>
        ...
        <input
          ...
          max={200}
          value={level}
          onChange={(e) => onLevel(Math.min(200, Math.max(1, Math.floor(Number(e.target.value) || 1))))}
        />
      </label>
      <label>
...
export function OddsCalculator() {
  ...
  const result = useMemo(
    () => crossOdds(a, b, lvlA + lvlB, optima),
    [a, b, lvlA, lvlB, optima],
  );
  ...
  return (
    <div className="pane planner">
      ...
      <p className="plan-note muted">
        ...
        probabilités réelles en jeu. Le résultat d'un croisement est une <b>distribution</b> pondérée
        par la <b>généalogie</b> (chaque monture = elle-même ×5 + ses 2 parents ×3 ; monocolore = 9,
        Dorée = 2, bicolore = 2). La génération la plus haute atteignable est la « cible » et reçoit
        p ; le reste se partage (1−p) au prorata des poids.
      </p>
    </div>
  );
}
