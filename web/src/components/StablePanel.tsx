...
function StableChip({ m }: { m: Dragodinde }) {
  ...
  return (
    ...
    >
      <b className="sc-name" style={{ color: GEN_COLOR[genOf(m.color)] }}>{m.name}</b>
      <span className="sc-meta muted">
        {m.color || "?"} · {m.sex === "F" ? "♀" : "♂"} · {STATUS_LABEL[m.status]}{m.keeper ? " · ★" : ""}
      </span>
    </div>
  );
}
  ...
  return (
    <section ref={setNodeRef} className={"pane stable-pane" + (isOver ? " drop-over" : "")}>
      <div className="pane-head">
        <h2>🏠 Étable</h2>
        <span className="muted">{stable.length} · {feconde} fécondes</span>
      </div>
      <input
      ...
      <div className="stable-chips">
        {filtered.length === 0 ? (
          <div className="empty">{stable.length === 0 ? "Étable vide." : "Aucune correspondance."}</div>
        ) : (
