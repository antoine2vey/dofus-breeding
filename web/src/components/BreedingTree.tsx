      ...
      <span className="tree-node">
        ...
        {s.count > 0 && (
          <span className="tree-count">
            ×{s.count < 10 ? s.count.toFixed(1) : Math.round(s.count)}
          </span>
        )}
        {p ? (
          <span
            className="tree-prob"
            style={{ color: s.prob >= 0.999 ? 'var(--accent)' : undefined }}
          >
      ...
      monteCarlo(
        {
          ...
          clonage,
          maxSteps: 0,
          inventory: useCheptel
            ? (JSON.parse(invSig) as ReturnType<typeof toSimInventory>)
            : undefined
        },
        300,
        makeRng(1234)
      ...
      <div className="plan-controls">
        ...
        <label className="chk" title="Démarre depuis tes montures réelles (grands-parents inclus)">
          <input
            type="checkbox"
            checked={useCheptel}
            onChange={(e) => setUseCheptel(e.target.checked)}
                    ...
                    >
                      ...
                      {inAnc && cnt > 0 && (
                        <span className="chip-count">
                          ×{cnt < 10 ? cnt.toFixed(1) : Math.round(cnt)}
                        </span>
                      )}
                    </button>
                  )
