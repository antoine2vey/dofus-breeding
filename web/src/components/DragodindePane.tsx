  ...
  return (
    <div className={"stat-mini" + (focused ? " focused" : "")}>
      ...
      <span className="quick">
        {isSer ? (
          <>
            <button className="q" title="Serenity −1" onClick={() => onStat("serenity", Math.max(lo, value - 1))}>
              −1
            </button>
            <button className="q" title="Serenity +1" onClick={() => onStat("serenity", Math.min(hi, value + 1))}>
              +1
            </button>
          </>
        ) : (
          <>
            ...
            <button className="q" title={`Set to ${meta.statMax / 1000}k`} onClick={() => onStat(stat.key, meta.statMax)}>
              {meta.statMax / 1000}k
            </button>
          </>
        )}
