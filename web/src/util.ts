...
export const moodFace = (serenity: number): string =>
  MOODS.find((m) => serenity >= m.min)!.face;
...
const etaToGoalSec = (fuel: number, stat: number, goal: number, dir: number, tickMs: number): number | null => {
  let f = fuel;
  let s = stat;
  let ticks = 0;
