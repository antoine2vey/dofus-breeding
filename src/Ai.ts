...
export interface AiActions {
  ...
  recordClone(p: {
    survivorId: number
    consumedId: number
  }): Promise<{ ok: boolean; cloneId?: number; error?: string }>
  addMounts(p: {
    color: string
    sex: 'M' | 'F'
    status: ReproStatus
  ...
  effect: Effect.gen(function* () {
    ...
    const reply = (
      ...
      const tools = {
        getState: tool({
          description:
            'Le cheptel + les enclos EN DIRECT : montures (couleur, sexe, état, keeper, enclos, grands-parents) et occupation des enclos.',
          inputSchema: z.object({}),
          execute: async () => actions.getState()
        }),
        getPlan: tool({
          description:
            "La source de vérité : feuille de route complète (besoins par génération, captures restantes) + la prochaine étape (élever / croiser / cloner / capturer) calculée depuis l'état réel.",
        ...
        simulate: tool({
          description:
            'Estimation Monte Carlo (depuis ton cheptel réel, grands-parents inclus) du nombre de montures pour atteindre une génération.',
          inputSchema: z.object({
        ...
        moveMounts: tool({
          description:
            "Déplace des montures vers un enclos (pour les monter féconde) ou vers l'étable (enclosId null). Sûr/réversible.",
        ...
        recordCross: tool({
          description:
            "Enregistre un croisement réel : crée le bébé (couleur/sexe obtenus) ; les deux parents deviennent stériles. À confirmer par l'utilisateur.",
          inputSchema: z.object({
            parentAId: num,
            parentBId: num,
            color: z.string(),
        ...
        recordClone: tool({
          description:
            "Enregistre un clonage : deux stériles de même GÉNÉRATION, un seul survit. « survivorId » est la monture conservée (redevient fertile, garde sexe/couleur/lignée) ; « consumedId » est détruite. Demande laquelle survit si l'utilisateur ne l'a pas précisé. À confirmer.",
          inputSchema: z.object({ survivorId: num, consumedId: num }),
          execute: async (p) => actions.recordClone(p)
        }),
        addMounts: tool({
          description:
            "Enregistre des captures/ajouts : N montures d'une couleur/sexe dans l'étable. Le sexe est obligatoire — demande-le si l'utilisateur ne l'a pas donné.",
          inputSchema: z.object({
            color: z.string(),
            sex: z.enum(['M', 'F']),
            status: status.default('fertile'),
