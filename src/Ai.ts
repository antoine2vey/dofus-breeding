...
export class Ai extends Effect.Service<Ai>()("app/Ai", {
  effect: Effect.gen(function* () {
    ...
    const modelId = yield* Config.string("OPENAI_MODEL").pipe(Config.withDefault("gpt-4o-mini"));
    ...
    const reply = (messages: ReadonlyArray<ChatMessage>, inventory: InvMount[], opts: ReplyOpts) => {
      const openai = createOpenAI({ apiKey });
      const tools = {
        getInventory: tool({
          description: "Le cheptel actuel : montures avec couleur, sexe, fécondité, keeper.",
        ...
        recommend: tool({
          description: "Le plan déterministe (croisements / captures / recyclages) depuis le cheptel.",
        ...
        simulate: tool({
          description: "Estimation Monte Carlo du nombre de montures pour atteindre une génération.",
          inputSchema: z.object({
          ...
          execute: async (a) => {
            const s = Core.monteCarlo(
              { targetGen: a.targetGen, level: a.level, optimakina: a.optimakina, clonage: a.clonage, maxSteps: 0 },
              200,
              Core.makeRng(1),
            );
            return {
        ...
        suggestName: tool({
          description: "Génère un nom de monture conforme à la convention (code couleur + sexe + n°).",
          inputSchema: z.object({
            color: z.string(),
            sex: z.enum(["M", "F"]),
          ...
          execute: async (a) => ({
            name: Core.buildName({ color: a.color, sex: a.sex, index: a.index, keeper: a.keeper }),
          }),
        }),
      };
