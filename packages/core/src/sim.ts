...
export function monteCarlo(cfg: SimConfig, runs: number, rng: () => number): SimSummary {
  ...
  return {
    ...
    breedings: {
      mean: mean(use.map((r) => r.breedings)),
      p50: pct(
        use.map((r) => r.breedings),
        0.5
      )
    },
    clonages: {
      mean: mean(use.map((r) => r.clonages)),
      p50: pct(
        use.map((r) => r.clonages),
        0.5
      )
    },
    raises: {
      mean: mean(use.map((r) => r.raises)),
      p50: pct(
        use.map((r) => r.raises),
        0.5
      )
